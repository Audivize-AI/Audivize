...
extension ASD {
    final class VideoBuffer: Utils.MLBuffer, Hashable, Equatable {        
        ...
        public private(set) var numNotBlank: Int = 0
        
        public var isBlank: Bool { numNotBlank <= 0 }
        
        public var hasEnoughFrames: Bool { numNotBlank >= ASDConfiguration.ASDModel.minFrames }
        
        public var isFull: Bool { numNotBlank >= ASDConfiguration.ASDModel.videoLength }
        
        private static let defaultBrightness: Float = 0.09047718613 // (110 / 255 - 0.1688) / 0.4161
        ...
        public func write(from pixelBuffer: CVPixelBuffer, croppedTo rect: CGRect, skip: Bool = false) {
            ...
            self.numNotBlank = min(self.numNotBlank + 1, ASDConfiguration.ASDModel.videoLength)
        }
        
        /// Write a blank frame to the buffer
        public func writeBlankFrame(skip: Bool = false) -> Void {
            // no need to keep adding blank frames buffer is full of them
            guard self.numNotBlank > 0 else { return }
            ...
            self.numNotBlank = max(self.numNotBlank - 1, 0)
        }
        ...
        public static func == (lhs: VideoBuffer, rhs: VideoBuffer) -> Bool {
